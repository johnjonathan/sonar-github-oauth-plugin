...
public class OAuthPlugin extends SonarPlugin {
    ...
    public static final class OAuthExtensions extends ExtensionProvider implements ServerExtension {
        ...
        private final Collection<Class<? extends GitHubOAuthClient>> oauthClients;

        public OAuthExtensions(org.sonar.api.config.Settings settings) {
            this.settings = settings;
            Reflections reflections = new Reflections("org.sonar.plugins.github.oauth.providers");
            this.oauthClients = reflections.getSubTypesOf(GitHubOAuthClient.class);
        }
        ...
        private Class<? extends GitHubOAuthClient> getOAuthClient() {
            Class<? extends GitHubOAuthClient> clazz = null;
            LOGGER.info("Registered Clients : {}", oauthClients);
            for (Class<? extends GitHubOAuthClient> oauthClientClass : oauthClients) {
